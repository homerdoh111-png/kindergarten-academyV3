...
const App = {
    ...
    async onReady() {
        ...
        if (typeof supabase !== 'undefined') {
            ...
            if (session) {
                ...
                setTimeout(() => {
                    ...
                    this.updateSpeechBubble(`Welcome back, ${this.state.username}! 🎉`);
                }, 2000);
                
                // Setup event listeners
                this.setupEventListeners();
                return; // EXIT HERE - don't show auth modal
            }
        } else {
            console.warn('⚠️ Supabase not available');
        }
        
        // Initialize 3D environment (for new users)
        setTimeout(() => {
            if (typeof TreehouseScene !== 'undefined') {
                TreehouseScene.init();
                console.log('🌳 Treehouse initialized');
        ...
        // Show auth modal for new users after environment loads
        setTimeout(() => {
            showAuthModal();
        }, 2000);
    ...
    setupEventListeners() {
        // Start Learning button (on name entry screen)
        const startBtn = document.getElementById('startBtn');
        const usernameInput = document.getElementById('usernameInput');
        
        if (startBtn && usernameInput) {
            startBtn.addEventListener('click', () => this.handleLogin());
...
document.getElementById('signupBtn')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (data.user) {
            ...
            // Initialize Buddy
            if (typeof Buddy3D !== 'undefined') {
                App.state.buddyInstance = Buddy3D.init('buddy-3d');
                console.log('🐻 Buddy initialized');
            }
            
            // Go directly to main screen
            App.showScreen('treehouseScreen');
            App.updateSpeechBubble(`Hi ${name}! Ready to learn? 🎉`);
            
            console.log('✅ Signed up successfully - loading main screen');
        }
...
document.getElementById('loginBtn')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (data.user) {
            ...
            // Go directly to main screen
            App.showScreen('treehouseScreen');
            App.updateSpeechBubble(`Welcome back, ${App.state.username}! 🎉`);
            
            console.log('✅ Signed in successfully - loading main screen');
        }
...
// Auth check is handled in App.onReady() above
